...
export function TeamView() {
  ...
  return <div className="team-section">
    ...
    <form className="card checkout-fields" onSubmit={e => { e.preventDefault(); setPending({ email: email.trim().toLowerCase(), role: nextRole }) }}>
      ...
      <label>Cargo<select className="input" value={nextRole} onChange={e => setNextRole(e.target.value as Role)}><option value="employee">Funcionário</option><option value="admin">Administrador</option><option value="blocked">Sem acesso</option></select></label>
      <button className="btn btn-primary" disabled={busy}>Definir acesso</button>
    </form>
    {message && <p className="card" role="status">{message}</p>}
    <div className="card"><h2 className="card-title">Equipe</h2>{!members.length ? <p>Carregando equipe…</p> : members.map(member => {
      const self = member.uid === authCurrentUser()?.uid
