...
export type SaleItemFull = { productId: string; name: string; qty: number; unitPrice: number; paid?: boolean }
export type Sale = {
...
export type Tab = 'dashboard' | 'vendas' | 'produtos' | 'relatorios' | 'clientes' | 'financeiro' | 'audit' | 'ingredientes' | 'pagamentos' | 'lucro'
...
export const salePaidAmount = (sale: Sale) => {
  if (sale.status === 'Presente') return 0
...
export const fmtDate = (iso: string) =>
  new Date(iso).toLocaleDateString('pt-BR', { timeZone: 'America/Sao_Paulo' }) + ' ' + new Date(iso).toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit', timeZone: 'America/Sao_Paulo' })
