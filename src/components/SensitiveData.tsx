    ...
    <section className="sensitive-locked" aria-label={label || 'Dados protegidos'}>
      <div className="sensitive-lock-card">
        ...
        <form onSubmit={event => { event.preventDefault(); void unlock() }} className="sensitive-unlock-form">
          <label className="sr-only" htmlFor={inputId}>Senha de acesso</label>
          <input id={inputId} className="pw-input" type="password" value={password} maxLength={128} autoComplete="current-password" placeholder="Senha de acesso" onChange={event => setPassword(event.target.value)} />
