export type CashPayment = { id: string; amount: number; description: string; person: string; date: string; archived?: boolean }
...
export const cashPaymentsTotal = (payments: CashPayment[]) => payments.filter(p => !p.archived).reduce((sum, p) => sum + Math.round(p.amount * 100), 0) / 100
